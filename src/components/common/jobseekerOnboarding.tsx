...
const STEP_TITLES = ["Profile", "Target Role", "Main Goal", "Complete Jobseeker Onboarding"];
...
export default function JobseekerOnboarding() {
  ...
  const { loading, jobseekerOnboarding } = useUserStore();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await jobseekerOnboarding(onboardingData);
    if (res.success) {
      if (res.next === '/') {
        router.replace('/');
      } else {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl rounded-2xl border border-neutral-200 bg-white p-4 shadow-lg md:p-8">
      ...
      <p className="mt-2 text-sm text-neutral-500">
        Complete all 4 screens to finish Jobseeker onboarding.
      </p>
      ...
      <form className="mt-8" onSubmit={handleSubmit}>
        ...
        {step === 4 && (
          <div className="space-y-4">
            <h3 className="text-base font-semibold text-neutral-800">Let&apos;s complete your Jobseeker onboarding</h3>
            <LabelInputContainer>
